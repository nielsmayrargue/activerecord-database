...
artists = db.execute ( "SELECT Name FROM Artist ORDER BY Name") do |row|
  puts row
end

# returns all the love songs
love_songs = db.execute ( "SELECT Name FROM Track WHERE Name LIKE '%love%' ") do |row|
  puts row
end

# returns all the tracks that are longer than 5 minutes.
long_tracks = db.execute ( "SELECT Name,Milliseconds FROM Track WHERE Milliseconds>3000000 ") do |row|
  puts row
end